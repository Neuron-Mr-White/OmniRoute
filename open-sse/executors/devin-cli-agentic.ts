...
function framePromptForNoToolsSummarizer(promptText: string): string {
  return [
    ...
    "COMPLETION RULE: the task is complete ONLY when the trace contains evidence for EVERY step and deliverable the user requested. A recent successful tool result is NOT completion. If the original request lists multiple steps and any step lacks trace evidence (a file that should exist, a command whose output is not yet shown, a verification not yet run), the task is INCOMPLETE — continue it by returning the <tool> envelope for the next pending step.",
    "When the task is genuinely complete, return only a concise final answer.",
